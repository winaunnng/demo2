...
class StockReportManager(models.Model):
    _name = 'stock.report.manager'
    ...
    summary = fields.Char()
    footnotes_ids = fields.One2many('stock.report.footnote', 'manager_id')
    company_id = fields.Many2one('res.company')

    def add_footnote(self, text, line):
        return self.env['stock.report.footnote'].create({'line': line, 'text': text, 'manager_id': self.id})

class StockReportFootnote(models.Model):
    _name = 'stock.report.footnote'
    _description = 'Stock Report Footnote'

    text = fields.Char()
    line = fields.Char(index=True)
    manager_id = fields.Many2one('stock.report.manager')

class StockCustomReport(models.AbstractModel):
    ...
    def _query_get(self, options, domain=None):
        ...
        self.env['stock.move.line'].check_access_rights('read')

        # query = self.env['stock.move.line']._where_calc(domain)


        # Wrap the query with 'company_id IN (...)' to avoid bypassing company access rights.
        # self.env['stock.move.line']._apply_ir_rules(query)
        return self.env['stock.move.line']._where_calc(domain).get_sql()
    ...
    def open_document(self, options, params=None):
        ...
        # Decode params
        model = params.get('model', 'stock.move.line')
    ...
    def _get_report_manager(self, options):
        ...
        existing_manager = self.env['stock.report.manager'].search(domain, limit=1)
